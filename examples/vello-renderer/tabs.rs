...
impl Tabs {
    ...
    #[allow(unused)]
    pub(crate) fn from_url<C: ModuleConfiguration>(url: Url, layouter: C::Layouter, handles: Handles<C>) -> Result<Self> {
        let mut tabs = SlotMap::new();
    ...
    pub fn open<C: ModuleConfiguration>(&mut self, url: Url, layouter: C::Layouter, handles: Handles<C>) -> Result<()> {
        let id = self.tabs.try_insert_with_key(|key| {
            EngineInstance::new_on_thread(url.clone(), layouter, kti(key), handles)
        })?;
