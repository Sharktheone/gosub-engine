...
use crate::traits::config::{HasDrawComponents, HasLayouter, HasRenderBackend, HasRenderTree};
...
pub trait TreeDrawer<C: HasDrawComponents>: WasmNotSend + 'static
{
    ...
    fn from_url(url: Url, layouter: C::Layouter, debug: bool) -> impl Future<Output =crate::types::Result<Self>> + WasmNotSend
    where
        Self: Sized;
