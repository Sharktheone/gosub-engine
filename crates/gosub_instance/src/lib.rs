...
impl<C: ModuleConfiguration> EngineInstance<C> {
    pub async fn new(url: Url, layouter: C::Layouter) -> Result<(Self, InstanceHandle)> {
        let (tx, rx) = tokio::sync::mpsc::channel(64);

        let instance = EngineInstance::with_chan(url.clone(), layouter, rx).await?;
    ...
    pub async fn with_chan(url: Url, layouter: C::Layouter, rx: Receiver<InstanceMessage>) -> Result<Self> {
    ...
    pub fn new_on_thread(url: Url, layouter: C::Layouter) -> Result<InstanceHandle>
    where
        C::Layouter: Send + 'static,
    {
        ...
        std::thread::spawn(move || {
            ...
            let mut instance = match rt.block_on(Self::with_chan(url, layouter, rx)) {
    ...
    fn handle_message(&mut self, message: InstanceMessage) {
        match message {
            InstanceMessage::Redraw(size) => {
                self.data.draw(size, &self.el);
            }
